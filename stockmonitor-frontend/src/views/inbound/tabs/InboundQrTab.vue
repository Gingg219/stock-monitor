...
import { CCard, CCardBody, CTable, CTableHead, CTableRow, CTableHeaderCell, CTableBody, CTableDataCell, CFormInput, CBadge, CButton, CAlert } from '@coreui/vue'
...
// income lấy từ store; ban đầu có thể null
const income = computed(() => store.current)

// --- plan khởi tạo sau khi lines có ---
const plan = ref([]) // [{ lineNo, pallet_size, box_size }]

watch(
  () => income.value?.lines,     // theo dõi khi lines sẵn sàng / thay đổi
  (lines) => {
    if (!lines) { plan.value = []; return }
    // nếu đã có plan thì giữ lại giá trị người dùng nhập; nếu chưa thì default
